...
UPDATE、DELETE和SELECT FOR UPDATE语句的WHERE条件列没有索引，会引起全表扫描，RC加记录锁，RR加Next-Key Lock，由于两阶段锁协议（2PL），不管记录是否符合WHERE中的条件，加锁后都会等到事物结束阶段才会释放锁。并发过程中，如果两个事物以不同的顺序加锁，会引起死锁。即使不引起死锁，也会导致类似锁表的现象。所以需要对WHERE中的条件列加索引。